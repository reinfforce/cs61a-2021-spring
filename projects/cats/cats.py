...
def accuracy(typed, reference):
    ...
    if type_len == 0 and refer_len == 0:
        return 100.0
    elif type_len == 0.0 and refer_len:
        return 0.0
    elif type_len and refer_len == 0.0:
        return 0.0
    else:
        for i in range(min(type_len,refer_len)):
            if typed_words[i] == reference_words[i]:
                true_list.append(1)
        return 100 * len(true_list) / type_len
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    ans, lowest_diff = min([(word,diff_function(typed_word,word,limit)) for word in word_list],key=lambda x:x[1])
...
def time_per_word(words, times_per_player):
    ...
    for time_per_player in times_per_player:
        ...
        for i in range(1,len(time_per_player)):
            time_diff.append(time_per_player[i] - time_per_player[i-1])
...
def fastest_words(match):
    ...
    for word in word_indices:
        fastest_player, min_time = min([(player,time(match,player,word)) for player in player_indices], key = lambda x:x[1])
        ans[fastest_player].append(word_at(match,word))
    return ans
    # END PROBLEM 10
