...
class Card:
    ...
    def power(self, opponent_card):
        ...
        """
        # BEGIN Problem 1
        self.mypower = self.attack - opponent_card.defense
        return self.mypower
        # END Problem 1
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if player.hand:
            for card in player.hand:
                powers.append(card.power(opponent_card))
            max_power = max(powers)
            max_power_index = powers.index(max_power)
            best_card = player.hand[max_power_index]
            self.attack += player.hand[max_power_index].attack
            self.defense += player.hand[max_power_index].defense
            player.hand.pop(max_power_index)
        # END Problem 5
        if best_card:
