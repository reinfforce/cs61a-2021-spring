...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
           return scheme_eval(scheme_apply(procedure, args, env), env)
        else:
            return scheme_apply(procedure, args.map(lambda item: scheme_eval(item, env)), env)
        # END PROBLEM 3
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        # END PROBLEM 2
    elif isinstance(procedure, LambdaProcedure): # also involves Macro!!! # 
        # BEGIN PROBLEM 9
